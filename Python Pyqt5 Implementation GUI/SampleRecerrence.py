...
from Sample import Ui_SampleDetect
class Ui_Recurrence(object):
    ...
    def dataHistory(self):
        ...
        global Mefalsepause,Breastquad ,Irradiat,Falsedecaps,Degmalig,Tumorsize ,Breast,Invnodes
        ...
        import Sample
        Sample.Mefalsepause=Mefalsepause
        Sample.Breastquad=Breastquad
        Sample.Falsedecaps=Falsedecaps
        Sample.Irradiat=Irradiat
        Sample.Degmalig=Degmalig
        Sample.Tumorsize=Tumorsize
        Sample.Breast=Breast
        Sample.Invnodes=Invnodes
    ...
    def Machinelearning(self):
        ...
        #global Mefalsepause,Breastquad ,Irradiat,Falsedecaps,Degmalig,Tumorsize ,Breast,Invnodes,sampleResult,      
        global sampleRecurrence  
        Mefalsepausei =self.comboBox_26.currentText()
        Mefalsepausei.map({'premefalse':1,'ge40':0,'lt40':3})
        Breastquadi=self.comboBox_27.currentText()
        Breastquadi.map({'left_up':7,'left_low':1,'right_up':9,'right_low':3,'central':5})
        Falsedecapsi=self.comboBox_24.currentText()
        Falsedecapsi.map({True:2,False:3})
        Irradiati =self.comboBox_25.currentText()
        Irradiati.map({True:2,False:3})
        Degmaligi =self.comboBox_22.currentText()
        Tumorsizei  =self.lineEdit_19.text()
        Breasti=self.comboBox_20.currentText()
        Breasti.map({'right':4,'left':6})
        invfalsedes = {'0-2':((0+2)/2),'3-5':((3+5)/2),'24-26':((24+26)/2),'9-11':((9+11)/2),'12-14':((12+14)/2),'15-17':((15+17)/2),'6-8':((6+8)/2)}
        Invnodesi =self.lineEdit_20.text()
        Invnodesi.map(invfalsedes)
        #print(Mitoses)
        #import the model
        print('beforemodel')
        model=joblib.load('recurrencemodel')
        print('aftermodel')
        result=model.predict([[Mefalsepausei,Breastquadi,Falsedecapsi,Irradiati,Degmaligi,Tumorsizei,Breasti,Invnodesi]])
        print(result)
        ...
        for i in range(len(result)):
            if(result[i]==4):
                ...
                BinaryOutput=result[i]
                print(BinaryOutput)
                #self.ShowMessageBox('Diagnosis',Output)
...
"}")
        ...
        self.DetectB.clicked.connect(self.dataHistory)
...
"}")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.DetectB.setToolTip(_translate("MainWindow", "Detect"))
